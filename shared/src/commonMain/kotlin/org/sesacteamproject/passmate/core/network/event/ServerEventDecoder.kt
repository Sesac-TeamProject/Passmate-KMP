...
object ServerEventDecoder {
    ...
    private fun decodeEvent(type: String, data: JsonElement): ServerEvent? {
        return when (type) {
            ...
            "GAME_STARTED" -> decodeData<ServerEvent.GameStarted>(data)
            ...
            "VOICE_HINT" -> decodeData<ServerEvent.VoiceHint>(data)
            "GAME_FINISHED" -> decodeData<ServerEvent.GameFinished>(data)
            "REPORT_READY" -> decodeData<ServerEvent.ReportReady>(data)
            "ROOM_CANCELLED" -> decodeData<ServerEvent.RoomCancelled>(data)
            "AI_FEEDBACK_READY" -> decodeData<ServerEvent.AiFeedbackReady>(data)
            "AI_FEEDBACK_FAILED" -> decodeData<ServerEvent.AiFeedbackFailed>(data)
