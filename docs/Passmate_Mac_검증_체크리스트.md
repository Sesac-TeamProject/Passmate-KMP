...
- [ ] **대기실 참가자 0명 회귀**(2026-09-08 수정): 초기 목록이 **WS 연결과 무관하게** 뜨는지 — 기내 모드로 WS만 끊고 진입해도 참가자가 보이거나 "참가자를 불러오지 못했어요 · 다시 시도"가 뜨고, **"학생 0명이 함께해요"로 둔갑하지 않아야** 한다 · 다시 시도 탭 → 재조회 · 이미 목록을 받은 뒤 잠깐 끊겼다 붙으면 인원수가 깜빡이지 않아야 한다
...
## 7-1. 상단 앱바 공통화 (2026-09-08)

- [ ] **뒤로가기 아이콘**: 모든 상세 화면에서 텍스트 `←`가 아니라 시안 `icon/arrow-left`(24pt·선 2pt·라운드 캡, ink #1B1F24)로 보이는지 — 특히 결과(에러)·정산(실패)·코인 내역
- [ ] **앱바 고정**: 아래 화면에서 본문을 스크롤해도 상단 앱바가 따라 올라가지 않는지 — 마이 · 참여한 방 · 내가 만든 방(탭 루트 3개) · 정산 · 유료 방 입장 · 방 리포트 · 진행 리모컨
- [ ] **여백·크기 통일**(2026-09-08): 앱바가 있는 모든 화면에서 제목이 **같은 높이**에 오는지(상단 12 고정) · 좌우 20 · 뒤로가기와 제목 간격 12 · 제목 크기가 24(탭 루트·명성·정산) 또는 20(그 밖) 둘 중 하나인지. **방 리포트는 18 → 20으로 커졌다.** 유료 방 입장 히어로와 진행 리모컨 헤더도 같은 상단 12로 올라왔다
- [ ] **탭 루트 로딩/에러 상태**: 로딩 중·실패 시에도 제목이 보이는지 (이전에는 성공 상태에서만 보였다)
- [ ] Desktop(`:composeApp:run`)에서도 같은지 — 앱바는 commonMain 공통 컴포넌트다
...
- [ ] ~~결제 수단 시트~~ — 화면 삭제(2026-09-08). 대신 **코인 충전에서 "충전"을 누르면 수단 선택 없이 바로 포트원 결제창**이 뜨는지 확인 `[백엔드]`
...
- [ ] `[백엔드]` **종료된 세션에서 대기실이 남지 않는지**(2026-09-08 수정): ①풀이 중 세션 종료 → 최종 순위에서 뒤로가기 → "입장 완료" 대기실이 뜨지 않고 결과로 · ②대기실에 있는 동안 선생님이 종료(SESSION_ENDED) → 바로 결과로 · ③이미 끝난 방 PIN으로 입장 → 대기실이 아니라 결과로 · ④역방향 확인: 풀이 중 세션이 끝나도 **최종 순위(M-05) 화면이 저절로 결과로 튕기지 않아야** 한다(대기실 구독이 살아 있어도 가로채지 않는다 — iOS는 `onDisappear` 미호출 시 특히 확인)
- [ ] **풀이 중 시스템 뒤로가기**(2026-09-08 추가, Android 전용): 문항 풀이 중 뒤로가기 → **대기실로 떨어지지 않고 "방을 나갈까요?" 확인 다이얼로그** · "계속 풀기"로 잔류 · "나가기"로 홈 복귀(참가 해제) · 최종 순위(M-05)에서 뒤로가기는 결과 화면으로 · 다이얼로그가 떠 있을 때 뒤로가기는 다이얼로그만 닫힘 (iOS는 `RouteStackLevel`이 `navigationBarBackButtonHidden(true)`라 시스템 뒤로가기 자체가 없어 대상 아님 · Desktop도 없음)
