...
class HostedRoomsViewModel(
    ...
    private fun onClickOngoingRoom(roomId: Long, pin: String) {
        viewModelScope.launch {
            _event.emit(HostedRoomsEvent.OpenSessionControl(roomId, pin))
        }
    }
    ...
    override fun onAction(action: HostedRoomsAction) {
        when (action) {
            ...
            is HostedRoomsAction.ClickOngoingRoom -> onClickOngoingRoom(action.roomId, action.pin)
